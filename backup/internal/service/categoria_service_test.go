// backend/internal/service/categoria_service_test.go
package service
...
type CategoriaServiceTestSuite struct {
	...
	mockRepo    *mocks.CategoriaRepositoryMock // Instancia de nuestro mock
...
func (s *CategoriaServiceTestSuite) SetupTest() {
	s.mockRepo = new(mocks.CategoriaRepositoryMock) // Crear nueva instancia del mock
...
func (s *CategoriaServiceTestSuite) TestGetAll_Success() {
	...
	mockCategorias := []domain.Categoria{
...
func (s *CategoriaServiceTestSuite) TestGetByID_Success() {
	...
	ctx := context.Background()
	mockID := uint(5)
	mockCategoria := &domain.Categoria{ID: mockID, Nombre: "Sopas", Slug: "sopas"}
...
func (s *CategoriaServiceTestSuite) TestGetByID_NotFound() {
	...
	s.ErrorIs(err, domain.ErrCategoriaNotFound) // Esperamos el error de DOMINIO traducido
	s.mockRepo.AssertExpectations(s.T())
}
...
func (s *CategoriaServiceTestSuite) TestCreate_Success() {
    ...
    s.mockRepo.On("Create", ctx, mock.MatchedBy(func(cat *domain.Categoria) bool {
...
func (s *CategoriaServiceTestSuite) TestCreate_NombreYaExiste() {
	...
	categoriaExistente := &domain.Categoria{ID: 1, Nombre: nombreLimpio, Slug: "postres"}
	...
	s.Error(err)
	s.Nil(nuevaCategoria)
	s.ErrorIs(err, domain.ErrCategoriaNombreYaExiste) // Esperamos el error de dominio específico
	s.mockRepo.AssertExpectations(s.T())
...
func (s *CategoriaServiceTestSuite) TestUpdate_Success() {
    ...
    categoriaExistente := &domain.Categoria{ID: id, Nombre: "Comida China", Slug: "comida-china"} // Nombre anterior
    ...
    s.mockRepo.On("Update", ctx, mock.MatchedBy(func(cat *domain.Categoria) bool {
...
func (s *CategoriaServiceTestSuite) TestDelete_Success() {
    ...
    ctx := context.Background()
    id := uint(1)
     categoriaExistente := &domain.Categoria{ID: id, Nombre: "A Borrar", Slug: "a-borrar"}
