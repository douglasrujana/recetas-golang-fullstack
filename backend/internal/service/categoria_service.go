...
import (
	// --- Imports necesarios ---
	"backend/internal/domain"     // El núcleo del negocio
	"backend/internal/repository" // La INTERFAZ del repositorio
	"context"
	"errors"
	"fmt"
...
type CategoriaService interface {
	...
	Create(ctx context.Context, input CategoriaInputDTO) (*domain.Categoria, error) // Devuelve la categoría creada
	Update(ctx context.Context, id uint, input CategoriaInputDTO) (*domain.Categoria, error) // Devuelve la categoría actualizada
...
type categoriaService struct {
	repo repository.CategoriaRepository // Dependencia de la interfaz del repo (Inyección)
...
func NewCategoriaService(repo repository.CategoriaRepository /*, logger *zap.Logger*/) CategoriaService {
...
func (s *categoriaService) GetByID(ctx context.Context, id uint) (*domain.Categoria, error) {
	categoria, err := s.repo.GetByID(ctx, id)
	if err != nil {
		if errors.Is(err, repository.ErrRecordNotFound) {
			return nil, domain.ErrCategoriaNotFound // Traducir error
		}
...
func (s *categoriaService) Create(ctx context.Context, input CategoriaInputDTO) (*domain.Categoria, error) {
...
func (s *categoriaService) Update(ctx context.Context, id uint, input CategoriaInputDTO) (*domain.Categoria, error) {
	nombreLimpio := strings.TrimSpace(input.Nombre)
	if nombreLimpio == "" {
		return nil, errors.New("servicio: el nombre de la categoría no puede estar vacío")
	}
